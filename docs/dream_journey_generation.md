...
many operations", "now versus later", or "take any number". Each
...
hook, route, status, and generated-object fields.
...
edits, shop economy, Dreamwell windows, statuses, delayed hooks, random
reveal/roll/wager envelopes, manifest-local generated objects, and complete
decision trees. These flags are debug surfaces for reproducible design
...
- Normal non-tree Journey sites usually show at least 2 root options. Compact
  commitment shapes can present one root option when the Journey decision is
  the visible commitment plus the standard Leave option. Some Journey Shapes may
  contain a bounded internal decision tree, but that tree must be fully visible
...
that they encountered "a now-versus-later scene" or "a take-any-number scene,"
not "a tuple of generated effects."
...
reacting to, it may deserve an explicit top-level shape.
...
post-entry target or detail variants, and similar structures. It does not permit hidden sequential Journey menus; true
