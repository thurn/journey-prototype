...
export type OneOperationManyTargetsReward<P extends TemplateParams = TemplateParams> = {
  ...
  readonly rollParams: (ctx: import("../../quest/context.js").JourneyContext, draw: DrawContext) => P;
  readonly operationKey: (params: P) => string;
  readonly targets: (params: P, ctx: import("../../quest/context.js").JourneyContext) => readonly TargetedRewardTarget[];
  readonly cec: (params: P, target: TargetedRewardTarget, ctx: import("../../quest/context.js").JourneyContext) => number;
  readonly render: (params: P, target: TargetedRewardTarget, ctx: import("../../quest/context.js").JourneyContext) => string;
  readonly operations: (
    params: P,
...
type AddSiteParamsForTarget = { scope: "current_dreamscape" | "next_dreamscape" };
const targetAddSite: OneOperationManyTargetsReward<AddSiteParamsForTarget> = {
  id: "target_add_site",
  weight: 0.5,
  targetKind: "route_site",
  symbols: ["reward", "route"],
  rollParams: (_ctx, draw) => ({
    scope: drawInt(draw, "oomt_add_site:scope", 0, 1) === 0
      ? "current_dreamscape"
      : "next_dreamscape",
  }),
  operationKey: (p) => `add_site:${p.scope}`,
  targets: (p) => JOURNEY_REWARDABLE_SITE_TYPES.map((siteType) => routeSiteTarget(siteType, p.scope)),
  cec: (p) => p.scope === "current_dreamscape" ? 100 : 75,
  render: (p, target) =>
    p.scope === "current_dreamscape"
      ? `Add a ${target.text} to this dreamscape`
      : `Add a ${target.text} to the next dreamscape you visit`,
  operations: (p, target, args) => routeAddOperations({
    ...args,
    target,
    templateId: targetAddSite.id,
    operationKey: targetAddSite.operationKey(p),
    scope: p.scope,
  }),
};
