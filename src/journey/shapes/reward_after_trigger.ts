...
export const rewardAfterTriggerPlugin = defineShapePlugin({
  ...
  scoreWeight: 1,
  repair: { actions: [{ action: "add_visible_trigger", kind: "reveal_hidden_target_or_outcome" }, { action: "store_reward_in_precommitted_delayed_metadata", kind: "repair_payload_family" }, { action: "replace_ambiguous_timing", kind: "repair_payload_family" }] },
});
