...
export function buildConservativeJourneyForShape(args: BuildArgs): JourneyManifest {
  ...
  const cardDelayedPrecommits = isNamedCardOperationMenuPayload(args.debugPayload)
    ...
    : [];
  const precommittedWithCardDelays = cardDelayedPrecommits.length > 0
    ? {
        ...filled.precommitted,
        delayed: [
    ...
    : filled.precommitted;
  const legacyPrecommitted = optionRouteEffects.length > 0 && precommittedWithCardDelays.routeEdits === undefined
    ? {
