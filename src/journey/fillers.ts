...
function semanticFingerprintFor(args: {
  ...
  const addChoiceBand = (count: unknown) => {
    if (typeof count === "number") {
      ...
      addBand({
        field: "choice_count",
        band,
        description: "Choice counts contribute only as single, narrow, menu, or broad semantic bands.",
      });
    }
  };
  const addChanceBand = (percent: unknown) => {
    if (typeof percent === "number") {
      ...
      addBand({
        ...
        band,
        description: "Percent chances contribute only as coarse probability bands.",
      });
    }
  };
  const addDurationBand = (durationKind: string, count: unknown) => {
    const band = semanticDurationBand(typeof count === "number" ? count : undefined, durationKind);
    addBand({
      field: "duration_count",
      band,
      description: "Durations contribute only as next, short-window, long-window, or unspecified bands.",
    });
  };
  const addResourceBand = (resource: unknown, amount: unknown, role: "cost" | "reward" | "burden") => {
    ...
    if (resource === "omens") {
      ...
      addBand({
        field: "omen_count",
        band,
        description: "Omen counts contribute only as none, single, multi, or major bands.",
      });
    } else {
      ...
      addBand({
        ...
        band,
        description: "Essence amounts contribute only as low, medium, high, or major bands.",
      });
    }
  };
  const addPayloadSemantics = (payload: Record<string, unknown>, role: "cost" | "reward" | "burden" | "neutral") => {
    ...
    addIdentity("dreamsign", payload.resultDreamsignId, payload.resultDreamsignName);
    addIdentity("bane", payload.baneName, payload.baneName);
    addIdentity("status", payload.statusId, payload.statusName);
    addIdentity("generated_object", payload.generatedObjectId, payload.generatedObjectName);
    ...
    addDurationBand(
      typeof payload.durationKind === "string" ? payload.durationKind : "duration",
      payload.durationCount ?? payload.count,
    );

    if (payload.kind === "gain_omens" || payload.kind === "omen_loss" || payload.kind === "omens") {
      addResourceBand("omens", payload.amount ?? payload.count, role === "neutral" ? "reward" : role);
    } else if (
      ...
      addResourceBand(payload.resource ?? "essence", payload.amount, role === "neutral" ? "reward" : role);
    }
  };
  const operationVerb = (operation: JourneyOperation): string => {
    ...
    if (operation.operationKind === "random_envelope" || operation.operationKind === "reveal_envelope") {
  ...
  const inspectOperation = (operation: JourneyOperation) => {
    ...
    if (operation.targetSelector) {
      ...
      if ("referenceKind" in operation.targetSelector && operation.targetSelector.referenceKind) {
        targetClasses.add(`${operation.targetSelector.selectorKind}:${operation.targetSelector.referenceKind}`);
      }
      if ("scope" in operation.targetSelector && typeof operation.targetSelector.scope === "string") {
      ...
      if ("ids" in operation.targetSelector) {
        operation.targetSelector.ids?.forEach((id) => addIdentity(operation.targetSelector!.selectorKind, id, undefined));
      }
      if ("names" in operation.targetSelector) {
        operation.targetSelector.names?.forEach((name) => addIdentity(operation.targetSelector!.selectorKind, undefined, name));
      }
    }

    for (const selected of operation.targetResolution?.selected ?? []) {
      addIdentity(operation.targetResolution?.selectorKind ?? selected.kind ?? "target", selected.id, selected.name);
    }
    ...
    if (operation.operationKind === "delayed_hook") {
      ...
      if (operation.triggerSelector) {
        ...
        addDurationBand(operation.triggerSelector.triggerKind, operation.triggerSelector.count);
      }
      if (operation.duration) {
        addDurationBand(operation.duration.durationKind, operation.duration.count);
      }
      if (operation.visibilityPolicy) {
        visibilityPolicies.add(`hook:${operation.visibilityPolicy.outcomeVisibility}`);
      }
    ...
    if (operation.resourceSemantics) {
      semanticValueBands.add(`${operation.resourceSemantics.resource}:${operation.resourceSemantics.amountKind}`);
      addResourceBand(
        ...
        operation.role === "cost" ? "cost" : operation.role === "burden" ? "burden" : "reward",
      );
    }
    ...
    addPayloadSemantics(operation.payload, operation.role === "cost" ? "cost" : operation.role === "burden" ? "burden" : operation.role === "reward" ? "reward" : "neutral");
  };
  const inspectUnknownPayload = (payload: unknown, role: "cost" | "reward" | "burden" | "neutral") => {
  ...
  for (const journeyOption of args.options) {
    ...
    journeyOption.costs.forEach((payload) => inspectUnknownPayload(payload, "cost"));
    journeyOption.effects.forEach((payload) => inspectUnknownPayload(payload, "reward"));
    journeyOption.burdens.forEach((payload) => inspectUnknownPayload(payload, "burden"));
    journeyOption.triggers.forEach((payload) => inspectUnknownPayload(payload, "neutral"));
    journeyOption.routeEffects.forEach((payload) => inspectUnknownPayload(payload, "neutral"));
  }
  ...
  for (const node of args.tree?.nodes ?? []) {
    ...
    for (const branch of node.branches) {
      motifs.add(`tree-branch:${branch.kind}:${branch.terminal?.outcome ?? branch.nextNodeId ?? "continue"}`);
      ...
      branch.effects.forEach((payload) => inspectUnknownPayload(payload, "reward"));
      branch.burdens.forEach((payload) => inspectUnknownPayload(payload, "burden"));
      branch.triggers.forEach((payload) => inspectUnknownPayload(payload, "neutral"));
      branch.routeEffects.forEach((payload) => inspectUnknownPayload(payload, "neutral"));
  ...
  for (const generatedObject of args.generatedObjects ?? []) {
    ...
    if (generatedObject.duration) {
      addDurationBand(generatedObject.duration.durationKind, generatedObject.duration.count);
    }
  }
  ...
  for (const outcome of args.precommitted.random ?? []) {
    ...
    if (
      ...
      visibilityPolicies.add(`random:${outcome.visibilityPolicy.outcomeVisibility}`);
    }
  }
  ...
  return {
    ...
    equivalenceBands: [...equivalenceBands.values()].sort((left, right) =>
      `${left.field}:${left.band}`.localeCompare(`${right.field}:${right.band}`, "en-US")
    ),
  };
}

export function withDistinctnessFingerprint(manifest: JourneyManifest): JourneyManifest {
