...
import type { ContentBundle, CardContent } from "../src/content/model.js";
...
// Minimal card content + draftPool entries so the fill's viability filter
// (which requires ≥ CARD_DRAFT_CHOICE_COUNT = 4 matches on draftPool) sees at
// least one viable card-draft reward pool. Without this, the empty-content
// stub leaves only pool 2 (delayed_bane → essence_gain) viable, which has
// only 2 distinct cost axes — too few for a 3-row fill.
function makeRichTestContext(seed: string) {
  const cards: CardContent[] = Array.from({ length: 4 }, (_, i) => ({
    id: `stub-card-${i}`,
    name: `Stub Card ${i}`,
    tides: [],
    rarity: "Common",
  ...
  return makeTestContext({
    ...
    content,
    stateOverrides: {
      draftPool: cards.map((c) => ({ cardId: c.id, copies: 1 })),
      draftPoolSummary: {
        totalCopies: cards.length,
        uniqueCards: cards.length,
        oneCopyCards: cards.length,
        twoCopyCards: 0,
      },
    },
  });
...
describe("randomTradesFill", () => {
  it("produces 2 or 3 options where each option's payloads come from a registered pool", () => {
    const { context, drawContext, stage } = makeRichTestContext("indep-1");
  ...
  it("two options never produce identical (cost, reward) tuples", () => {
    const { context, drawContext, stage } = makeRichTestContext("indep-2");
    const filled = randomTradesFill({ context, drawContext, stage })!;
